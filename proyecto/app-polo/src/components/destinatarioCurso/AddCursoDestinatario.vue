    ...
    <div class="column q-pa-lg">
      <q-card square class="shadow-24" style="width: 300px; height: 100%">
        <q-card-section class="bg-deep-purple-7">
          <h4 class="text-h5 text-white q-my-md">Nuevo Curso Destinatario</h4>
        </q-card-section>
        <q-card-section>
    ...
    return {
      async addCursoDestinatario() {
        const destinatarioCursoNew = {
          ...
          nombreMostrar: `${cursoId.value.nombre} - ${destinatarioId.value.nombre}`,
        };
        console.log(`add curso ${JSON.stringify(destinatarioCursoNew)}`);
        await api
